class BattleShipGame {
	...
	strike(i, j) {
		...
		if(cellType === this._CELL_TYPES.SHIP) {
			...
			if(this._checkState()) {
				...
				return this._STATE.WOUNDED;
			}
		} else {
			this._map[i][j] = cellType;
			this._update();
			
			return this._STATE.MISS;
		}
	...
	_checkState(resolution = false) {
		var states = this._ships.states;
		var state = false;
		var coordinates = this._ships.coordinates;
		var currentShip = this._currentShip;

		if(states[currentShip].every(state => !state)) {
			if(resolution) this._setSpace(coordinates[currentShip]);
